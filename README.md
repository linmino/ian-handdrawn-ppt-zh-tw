# Ian Handdrawn PPT（繁體中文版本）

> 這是 `helloianneo/ian-handdrawn-ppt` 的繁體中文版本 fork。
>
> 把文章、課程筆記、提綱或既有材料，整理成一套繁體中文手繪技術解說風格的 PPT-style 頁面圖。
>
> 21:9 封面 | 16:9 內文配圖 | PNG 輸出 | 適合文章配圖、課程教材、知識卡片和技術概念解釋

---

## 這個倉庫是什麼

這個倉庫是 `helloianneo/ian-handdrawn-ppt` 的**繁體中文版本 fork**。

Ian Handdrawn PPT 是一個 Codex Skill，用來指導 AI Agent 把內容整理成**繁體中文手繪技術解說圖**。

它不是傳統 PPT 模板，也不是可編輯 PPTX 生成器。它的目標是產出可以直接用於文章、課程、分享內容裡的**整頁 PNG 視覺圖**：
...
一句話：**讓 AI 不只是“做幾頁 PPT”，而是先想清楚內容，再生成一組有統一風格的繁體中文技術解說圖。**

推薦生圖模型：**最好使用 ChatGPT Image 2.0**。如果當前環境沒有這個模型，就使用可用的最高品質影像模型，並儘量讓整套圖保持同一個模型、同一套 style lock。
...
- 寫中文技術文章，需要封面圖和內文配圖的人
- 製作課程、訓練營、工作坊內容，需要解釋複雜概念的人
- 想把長文、提綱、講稿轉成視覺化頁面的人
...
這個 skill 預設使用 Ian 的繁體中文手繪技術解說風格：
...
```text
ian-handdrawn-ppt-zh-tw/assets/reference-handdrawn-article-illustration-style.png
```
...
```bash
git clone https://github.com/linmino/ian-handdrawn-ppt-zh-tw.git
cd ian-handdrawn-ppt-zh-tw
```
...
cp -R ./ian-handdrawn-ppt-zh-tw "${CODEX_HOME:-$HOME/.codex}/skills/"
```

安裝後，在 Codex 裡使用：

```text
Use $ian-handdrawn-ppt-zh-tw 把這篇文章做成 1 張繁體中文封面圖 + 3 張繁體中文內文配圖。
```
...
```text
Use $ian-handdrawn-ppt-zh-tw 把下面這篇文章做成 1 張 21:9 繁體中文封面圖 + 3 張 16:9 繁體中文內文配圖。
風格保持繁體中文手繪技術解說圖，文字儘量短，每張圖只表達一個觀點。
...
```text
Use $ian-handdrawn-ppt-zh-tw 把這份課程大綱整理成 8 頁繁體中文手繪技術教材圖。
對象是已有基礎的新手，每頁一個核心概念，輸出 slide-by-slide blueprint，然後生成最終 PNG 頁面圖。
...
```text
Use $ian-handdrawn-ppt-zh-tw 先不要生圖。
